...
namespace Microsoft.AspNetCore.Server.Kestrel.FunctionalTests
{
    public class AddressRegistrationTests
    {
        ...
        public async Task RegisterAddresses_HostName_Success()
        {
            ...
            await RegisterAddresses_Success($"http://{hostName}:0", $"http://{hostName}", appendPortToTestUrl: true);
        }
        ...
        {
            await RegisterAddresses_Success(addressInput, testUrl, appendPortToTestUrl: true);
        }
        ...
        {
            await RegisterAddresses_Success(addressInput, testUrl, appendPortToTestUrl: false);
        }
        ...
        {
            var address = $"http://127.0.0.1:{GetNextPort()}";
            await RegisterAddresses_Success_WithRetry(address, address, appendPortToTestUrl: false);
        }
        ...
        public async Task RegisterAddresses_IPv4LocalhostStaticPort_Success()
        {
            ...
            await RegisterAddresses_Success_WithRetry(address, new[] { address, $"http://127.0.0.1:{port}" }, appendPortToTestUrls: false);
        }
        ...
        {
            var port = GetNextPort();
            await RegisterIPEndPoint_Success_WithRetry(new IPEndPoint(IPAddress.Loopback, port), $"http://127.0.0.1:{port}", appendPortToTestUrl: false);
        }
        ...
        {
            var port = GetNextPort();
            await RegisterIPEndPoint_Success_WithRetry(new IPEndPoint(IPAddress.IPv6Loopback, port), $"http://[::1]:{port}", appendPortToTestUrl: false);
        }
        ...
        {
            await RegisterIPEndPoint_Success(endPoint, testUrl, appendPortToTestUrl: true);
        }
        ...
        {
            await RegisterIPEndPoint_Success(endpoint, testUrl, appendPortToTestUrl: false);
        }
        ...
        {
            await RegisterAddresses_Success(addressInput, testUrls, appendPortToTestUrls: true);
        }
        ...
        {
            await RegisterAddresses_Success(addressInput, testUrls, appendPortToTestUrls: false);
        }
        ...
        {
            var address = $"http://[::1]:{GetNextPort()}";
            await RegisterAddresses_Success_WithRetry(address, address, appendPortToTestUrl: false);
        }
        ...
        public async Task RegisterAddresses_IPv6LocalhostStaticPort_Success()
        {
            var port = GetNextPort();
            var address = $"http://localhost:{port}";
            await RegisterAddresses_Success_WithRetry(address, new[] { address, $"http://127.0.0.1:{port}", $"http://[::1]:{port}" }, appendPortToTestUrls: false);
        }

        private async Task RegisterAddresses_Success(string addressInput, string[] testUrls, bool appendPortToTestUrls)
        {
            ...
            using (var host = hostBuilder.Build())
            {
                ...
                foreach (var testUrl in (appendPortToTestUrls ? testUrls.Select(testUrl => $"{testUrl}:{host.GetPort()}") : testUrls))
                {
        ...
        private Task RegisterAddresses_Success(string addressInput, string testUrl, bool appendPortToTestUrl)
            => RegisterAddresses_Success(addressInput, new[] { testUrl }, appendPortToTestUrl);

        private Task RegisterAddresses_Success_WithRetry(string addressInput, string[] testUrls, bool appendPortToTestUrls)
            => Register_Success_WithRetry(RegisterAddresses_Success(addressInput, testUrls, appendPortToTestUrls));

        private Task RegisterAddresses_Success_WithRetry(string addressInput, string testUrl, bool appendPortToTestUrl)
            => RegisterAddresses_Success_WithRetry(addressInput, new[] { testUrl }, appendPortToTestUrl);

        private async Task RegisterIPEndPoint_Success(IPEndPoint endPoint, string testUrl, bool appendPortToTestUrl)
        {
            var hostBuilder = new WebHostBuilder()
                .UseLoggerFactory(_loggerFactory)
                .UseKestrel(options =>
            ...
            using (var host = hostBuilder.Build())
            {
                ...
                var testUrlWithPort = appendPortToTestUrl ? $"{testUrl}:{host.GetPort()}" : testUrl;
        ...
        private Task RegisterIPEndPoint_Success_WithRetry(IPEndPoint endPoint, string testUrl, bool appendPortToTestUrl)
            => Register_Success_WithRetry(RegisterIPEndPoint_Success(endPoint, testUrl, appendPortToTestUrl));

        private async Task Register_Success_WithRetry(Task registerTask)
        {
            var retryCount = 0;
